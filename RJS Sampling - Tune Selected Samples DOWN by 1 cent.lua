...
one_cent_down_factor = (1 - linear_cent_approximation)
...
function tune_selected_samples_down(cents, tune_mode)
		...
		for i = 0, num_samples - 1, 1 do
			...
			if reaper.IsMediaItemSelected(sample) then
				if tune_mode == "pitch" then
					...
					reaper.SetMediaItemTakeInfo_Value(reaper.GetTake(sample, 0), "D_PLAYRATE", current_value * one_cent_down_factor)
					reaper.UpdateItemInProject(sample)	
					update_group_member_items(sample, current_value * one_cent_down_factor, tune_mode)
				end
			end
		end
	end
...
function main()
	...
	tune_selected_samples_down(1, tune_mode)
end
