	...
	if group_id ~= 0 then
		for i = reaper.CountMediaItems(0) - 1, 0, -1 do
			...
			if temp_id == group_id and temp_item ~= item then
				...
				reaper.SetMediaItemTakeInfo_Value(reaper.GetTake(temp_item, 0), "D_STARTOFFS", reaper.GetMediaItemTakeInfo_Value(reaper.GetTake(temp_item, 0), "D_STARTOFFS") + amount)
			end			
		end		
	end	
end

function decrease_leading_pad_length(item, amount)
    ...
    for i = 0, item_count - 1, 1 do
      ...
      if sel_item_track ~= reaper.GetTrack(0, 0) and notes_numbercodes[2] == notes_numbercodes[3] or sel_item_track == reaper.GetTrack(0, 0) and notes_numbercodes[1] == notes_numbercodes[4] then  
        if sel_item_track == reaper.GetTrack(0, 0) then
          local temp = item
          item = target_item
          target_item = temp
		...
		local target_item_length = reaper.GetMediaItemInfo_Value(target_item, "D_LENGTH")
		
		if target_item_length > amount then
			...
			update_group_member_items(target_item, amount)
		end
        return 0;
      end
    end  
...
function main()
	...
	for i = 0, selected_item_count - 1, 1 do
		...
		decrease_leading_pad_length(item, 0.0005)
	end
end
